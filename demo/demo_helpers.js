...
function parseImdDecileMatchExpression(expr) {
  ...
  function scheduleLegendUpdate(map) {
    if (!map.isStyleLoaded()) {
      map.once('idle', () => updateImdLegend(map));
      return;
    }
    window.requestAnimationFrame(() => updateImdLegend(map));
  }
  
  function setupImdLegend(map) {
    map.on('styledata', (e) => {
  ...
  export { setupImdLegend, scheduleLegendUpdate, setupA11yDiscoveryCallout };